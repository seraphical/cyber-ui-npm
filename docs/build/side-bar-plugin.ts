...
const getComponentsSideBar = async (path: string) => {
  ...
  return Promise.all(
    dirArr.map(async (dirItemPath: string) => {
      ...
      return {
        ...
        items: await Promise.all(
          fileArr.map(async (fileName: string) => {
            ...
            const sideName =
              reg.exec(fileStr) && (reg.exec(fileStr) as Array<string>)[0];
            return {
              text: sideName,
              link: `/zh-CN/components${path}/${dirItemPath}/${
                fileName.split('.')[0]
              }`,
            };
          }),
...
const mdFilePath = ['/vue3', '/vue2', '/react'];
